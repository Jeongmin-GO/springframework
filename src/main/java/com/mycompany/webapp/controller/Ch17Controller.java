...
public class Ch17Controller {
	...
	public String join(Ch14Member member, Model model) {
		...
		//패스워드 암호화
		String mpwd = member.getMpassword(); //입력한 패스워드 받기
		BCryptPasswordEncoder pwdEncoder = new BCryptPasswordEncoder(); //암호화 해주는 인코더 생성
		mpwd = "{bcrypt}"+pwdEncoder.encode(mpwd);
		member.setMpassword(mpwd);
		
		//회원가입 처리
		JoinResult jr = mservice.join(member);
	...
	public String userInfo(Authentication authentication) {
		...
		//사용자 아이디 얻기
		String mid = authentication.getName();//스프링 시큐리티에서 인증을 얻어오는 메서드 이름(getName)
		
		//사용자 권한(롤)이름 얻기
		List<String> authorityList = new ArrayList<>();
		//getAuthorities의 return값이 Collection<? extends GrantedAuthority>
